...
const DailyReportAdmin = () => {
    ...
    const retrieveItem = () => {
        daily_reportService.getAll().then(
            (response) => {
    ...
    const columns = [
            ...
            cell: (row) => {
                return (
                    ...
                    {row.draft_file_name ? (
                        ...
                        <React.Fragment>
                            <Link to={"/daily-report-admin/draft/" + row.id} title="Upload draft file">
                                <IconButton color="primary" aria-label="Edit">
                                        <CloudUploadIcon size="small" color="secondary" />
                                </IconButton>
                            </Link>
                        </React.Fragment>
                    )}
                ...
                return (
                    <React.Fragment>
                        {row.release_file_name ? (
                            ...
                            <React.Fragment>
                                <Link to={"/daily-report-admin/release/" + row.id} title="Upload release file">
                                    <IconButton color="primary" aria-label="Edit">
                                        <CloudUploadIcon size="small" color="secondary"/>
                                    </IconButton>
                                </Link>
        ...
        },
        {
            ...
            cell: (row) => {
                return (
                    <React.Fragment>
                        <Link to={"/daily-report-admin/form/" + row.id} title="Detail">
                            <IconButton color="primary" aria-label="Edit">
                                <Edit size="small" />
                            </IconButton>
                        </Link>
                        <IconButton color="primary" title="Delete" aria-label="Delete" onClick={() => {
                            ...
                            if (r == true) {
                                daily_reportService.remove(row.id)
                                    .then((response) => {
        ...
        <Container className={classes.container} maxWidth="xl">
            <SparepartsContext.Provider value={value}>
                ...
                {!user.roles.includes("ROLE_ADMIN") ? (
                    ...
                    <React.Fragment>
                        <Typography variant="h4" className={classes.title}>Daily Report</Typography>
                        <Card>
                            <CardContent>
                ...
                <Link to={"/daily-report-admin/form"}>
