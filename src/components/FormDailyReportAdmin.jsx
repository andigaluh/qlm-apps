...
const FormDailyReportAdmin = () => {
    ...
    /* const updateItem = (id, values, setSubmitting) => {
        ...
        setTimeout(() => {
            ...
            data.append("status", values.status);
            data.append("type_report", values.type_report);
            daily_reportService.update(id, data)
                .then(
                    (response) => {
    ...
    const updateItem = (id, values, setSubmitting) => {
        ...
        setTimeout(() => {
            ...
            daily_reportService.update(id, values)
                .then(
                    (response) => {
                        console.log(response.data);
                        setSubmitting(false);
            ...
            {!user.roles.includes("ROLE_ADMIN") ? (
                ...
                <React.Fragment>
                    <div className={classes.titleContainer}>
                        ...
                        <Typography variant="h4" className={classes.title}>Daily report's Form</Typography>
                    </div>
                    <Card>
                        <CardContent>
                            <Formik
                                ...
                                validationSchema={Yup.object({
                                    ...
                                    //draft_file_name: (isAddMode) ? Yup.string().required("Required") : null,
                                })}
                                            ...
                                            {/* <Grid item xs={12}>
                                                <div className={classes.item}>
                                                    <TextField
                                                        ...
                                                        id="draft_file_name"
                                                        label="Upload File"
                                                        autoComplete="draft_file_name"
                                                        type="file"
                                                        onChange={(event) => {
                                                            formik.setFieldValue("draft_file_name", event.target.files[0]);
                                                        }}
                                                        InputLabelProps={{
                                                            shrink: true,
                                                        }}
                                                    />
                                                    {formik.touched.draft_file_name && formik.errors.draft_file_name ? (
                                                        <Typography size="small" color="error">{formik.errors.draft_file_name}</Typography>
                                                    ) : null}
...
export default FormDailyReportAdmin;