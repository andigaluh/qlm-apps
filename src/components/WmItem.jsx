...
const WmItem = () => {
    ...
    const retrieveItem = () => {
        wm_item_checkService.getAll().then(
            (response) => {
    ...
    const columns = [
        ...
        {
            name: "Type",
            selector: (row) => row.wm_item_check_category.wm_type.name,
            sortable: true,
        },
        {
            name: "Category",
            selector: (row) => row.wm_item_check_category.name,
            sortable: true,
        },
        {
            name: "name",
        ...
        },
        {
            ...
            cell: (row) => {
                return (
                    <React.Fragment>
                        <Link to={`/wm-item/form/${row.id}`} title="Detail">
                        ...
                        <IconButton color="primary" title="Delete" aria-label="Delete" onClick={() => {
                            ...
                            if (r == true) {
                                wm_item_checkService.remove(row.id)
                                    .then((response) => {
                                        setOpen(true);
        ...
        <Container className={classes.container}>
            <ToolsContext.Provider value={value}>
                ...
                {((!user.roles.includes("ROLE_ADMIN"))) ? (
                    ...
                    <React.Fragment>
                        <Typography variant="h4" className={classes.title}>Washing Machine Item</Typography>
                        <Card>
                            <CardContent>
