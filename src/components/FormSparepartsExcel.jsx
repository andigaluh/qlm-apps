...
const FormSparepartsExcel = () => {
    ...
    const createItem = (values, setSubmitting, resetForm) => {
        ...
        setTimeout(() => {
            ...
            parts_excelService.create(data, (event) => {
    ...
    useEffect(() => {
        if (!isAddMode) {
            parts_excelService.get(docId).then(
                (response) => {
                    const responseData = {
                        id: response.data.id,
            ...
            {!user.roles.includes("ROLE_SUPERVISOR") ? (
                ...
                <React.Fragment>
                    <div className={classes.titleContainer}>
                        ...
                        <Typography variant="h4" className={classes.title}>Spareparts excel's Form</Typography>
                    </div>
                    <Card>
                        <CardContent>
                            <Formik
                                        ...
                                        <Grid container spacing={2}>
                                            <Grid item xs={12}>
                                                <Typography variant="body2">Please use this xls template file before upload : &nbsp;
                                                    <a href={`${process.env.REACT_APP_UPLOADS}spareparts_template.xlsx`} target="_blank" className={classes.link}>
                                                        <Button variant="contained" className={classes.buttonContainer} >
                                                            <CloudDownload className={classes.button} /> spareparts_template.xlsx
                                                        </Button>
                                                    </a>
                                                </Typography>
                                            </Grid>
...
export default FormSparepartsExcel;